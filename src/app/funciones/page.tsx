...
export const metadata = {
  ...
  description: 'Explora las 15+ funcionalidades del ecosistema starTAP: Hardware NFC Gratuito + Plataforma Inteligente Pro para Google Maps.',
};
...
export default function FuncionesPage() {
          ...
          mockup: (
            <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-lg space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                <span className="px-2.5 py-1 rounded-lg bg-emerald-100 text-emerald-800 text-xs font-bold flex items-center">
                  💚 Excelente atención (+54)
                </span>
                <span className="px-2.5 py-1 rounded-lg bg-emerald-100 text-emerald-800 text-xs font-bold flex items-center">
                  💚 Comida deliciosa (+42)
                </span>
                <span className="px-2.5 py-1 rounded-lg bg-amber-100 text-amber-900 text-xs font-bold flex items-center">
                  ⭐ Ambiente acogedor (+28)
                </span>
                <span className="px-2.5 py-1 rounded-lg bg-red-100 text-red-800 text-xs font-bold flex items-center">
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
      ...
      <section className="pt-28 pb-16 bg-gradient-to-b from-gray-900 via-gray-900 to-gray-800 text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-brand-500/20 border border-brand-400/30 text-brand-300 text-xs font-semibold uppercase tracking-wider mb-6">
            ...
            <span>Suite Completa de Funciones starTAP</span>
          </div>
          ...
          <h1 className="text-3xl md:text-5xl lg:text-6xl font-extrabold tracking-tight max-w-4xl mx-auto leading-tight mb-6">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-300 via-brand-200 to-amber-300">
              superpoderes para tu ficha de Google
            </span>
          </h1>

          <p className="text-lg md:text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed mb-10">
            Desde responder reseñas con Inteligencia Artificial hasta medir el rendimiento de tus meseros con tarjetas NFC. Todo diseñado respetando la identidad de tu marca en Panamá.
          </p>

          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              href="/shop"
              className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-brand-500 hover:bg-brand-400 text-brand-950 font-bold text-sm uppercase tracking-wider transition-all shadow-lg flex items-center justify-center space-x-2"
            >
            ...
            >
              <Sparkles className="h-4 w-4 text-amber-400" />
              <span>Conocer Software Pro</span>
            </Link>
          </div>
        </div>
      </section>

      {/* Feature Section Navigation Sticky Bar */}
      <section className="sticky top-16 z-30 bg-white/90 backdrop-blur-md border-b border-gray-200 shadow-sm py-3.5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-start md:justify-center space-x-2 overflow-x-auto no-scrollbar">
            {featureGroups.map((group, idx) => (
              <a
                key={idx}
                href={`#grupo-${idx}`}
                className="px-4 py-2 rounded-lg text-xs md:text-sm font-semibold whitespace-nowrap bg-gray-100 text-gray-700 hover:bg-brand-50 hover:text-brand-950 transition-colors"
              >
                {group.badge}
              </a>
            ))}
      ...
      {/* Main Alternating Split-Feature Blocks */}
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-28">
        {featureGroups.map((group, groupIdx) => (
          <section id={`grupo-${groupIdx}`} key={groupIdx} className="scroll-mt-36 space-y-16">
            {/* Group Header */}
            <div className="text-center max-w-3xl mx-auto border-b border-gray-200 pb-8">
              <span className="text-xs font-extrabold uppercase tracking-wider text-brand-600 px-3 py-1 bg-brand-50 rounded-full border border-brand-200">
                {group.badge}
              </span>
              <h2 className="text-2xl md:text-4xl font-extrabold text-gray-900 mt-4">{group.title}</h2>
              <p className="text-gray-600 mt-2 text-sm md:text-base">{group.description}</p>
            </div>
              ...
              {group.features.map((feat, featIdx) => {
                ...
                return (
                  ...
                  >
                      ...
                      <ul className="space-y-2.5 pt-2">
                        {feat.bullets.map((b, bIdx) => (
                          <li key={bIdx} className="flex items-start text-xs sm:text-sm text-gray-700">
                            <CheckCircle2 className="h-4 w-4 text-brand-500 mr-2.5 shrink-0 mt-0.5" />
                            <span>{b}</span>
                          </li>
                        ))}
                      </ul>

                      {/* Impact / Para qué Callout Box */}
                      <div className="pt-4 border-t border-gray-200">
                        <div className="bg-brand-50/70 border border-brand-200 rounded-xl p-4 text-xs text-gray-800">
                    ...
                    {/* UI Mockup Visual Column (6 cols) */}
                    <div className={`lg:col-span-6 ${isEven ? '' : 'lg:order-1'}`}>
                      <div className="bg-gradient-to-br from-gray-100 to-gray-200 p-6 md:p-8 rounded-3xl border border-gray-200 shadow-sm relative overflow-hidden">
                        <div className="absolute top-0 right-0 -mr-16 -mt-16 w-48 h-48 bg-brand-500/10 rounded-full blur-2xl pointer-events-none" />
                        {feat.mockup}
                      </div>
                    </div>
                  </div>
      ...
      <section className="bg-brand-950 text-white py-16 border-t border-brand-900">
        <div className="max-w-5xl mx-auto px-4 text-center">
          ...
          <p className="text-brand-200 text-sm md:text-base max-w-2xl mx-auto mb-8">
            Empieza con tus tarjetas y placas NFC físicas sin pagar mensualidades obligatorias, y activa la Inteligencia Artificial cuando quieras escalar.
          </p>
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
            ...
            <a
              href="https://wa.me/50767134341?text=Hola,%20quisiera%20asesoría%20sobre%20las%20funciones%20de%20starTAP"
              target="_blank"
