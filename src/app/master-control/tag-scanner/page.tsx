      ...
      ndef.addEventListener('reading', ({ message, serialNumber }: any) => {
        ...
        for (const record of message.records) {
          if (record.recordType === 'url' || record.recordType === 'text') {
            ...
            const match = text.match(/STT-\d+/i);
            if (match) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-6 font-sans">
      <div className="max-w-md mx-auto space-y-5">
        ...
        {/* 1. Buscador Ultrarrápido de TAG */}
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl space-y-3">
          <label className="block text-xs font-bold text-amber-400 uppercase tracking-wider">
            1. Localizar Código de TAG (STT-XXXX)
          </label>
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSearch(); }} className="flex gap-2">
            <div className="relative flex-grow">
              <input
                ...
                placeholder="Ej: STT-1001 o 1001"
          ...
          {/* Quick Buttons por lotes frecuentes */}
          <div className="flex flex-wrap gap-1.5 pt-1">
            <span className="text-[10px] text-slate-500 font-semibold self-center mr-1">Frecuentes:</span>
            {['STT-1001', 'STT-1002', 'STT-1003', 'STT-1004'].map((code) => (
              <button
                key={code}
                type="button"
                onClick={() => { setSearchCode(code); handleSearch(code); }}
                className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-[10px] font-mono font-bold transition"
              >
                {code}
              </button>
            ))}
